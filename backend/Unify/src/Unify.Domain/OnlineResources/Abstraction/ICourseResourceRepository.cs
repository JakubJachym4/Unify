...
public interface ICourseResourceRepository
{
    ...
    Task<CourseResource?> GetByIdAsyncIncludeAttachments(Guid id, CancellationToken cancellationToken);
    ...
    Task<List<CourseResource>> GetByCourseAsyncIncludeAttachments(Course course, CancellationToken cancellationToken);
}